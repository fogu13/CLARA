...
const SYSTEM_PROMPT = `You are an expert EU regulatory compliance analyst specialising in the EU AI Act (Regulation 2024/1689) and the General Data Protection Regulation (GDPR, Regulation 2016/679). You assess marketing campaigns and use cases for compliance.
...
serve(async (req) => {
  ...
  try {
    const { campaignDescription, title, additionalDocuments } = await req.json();

    let userPrompt = `Please assess the following marketing campaign or use case for EU AI Act and GDPR compliance:\n\n**Title:** ${title}\n\n**Description:**\n${campaignDescription}`;

    if (additionalDocuments && additionalDocuments.length > 0) {
      userPrompt += `\n\n**Additional Local Compliance Documents:**\n`;
      additionalDocuments.forEach((doc: { name: string; content: string }) => {
        userPrompt += `\n--- Document: ${doc.name} ---\n${doc.content}\n`;
      });
    }
        ...
        function: {
          ...
          parameters: {
            ...
            properties: {
              ...
              summary: {
                type: "string",
                description: "Executive summary of the compliance assessment (2-3 sentences)",
              },
              gdpr_score: {
                type: "integer",
              ...
              eu_ai_act_score: {
                type: "integer",
                description: "EU AI Act compliance score 0-100",
              },
              findings: {
                type: "array",
                ...
                items: {
                  ...
                  properties: {
                    ...
                    area: { type: "string", description: "The specific regulation area (e.g. Lawful Basis, Consent, Transparency)" },
                    article: { type: "string", description: "Relevant article reference (e.g. Art. 6 GDPR)" },
                    status: { type: "string", enum: ["pass", "fail", "warning", "not_applicable"] },
                    score: { type: "integer", description: "Score for this specific area 0-100" },
                    finding: { type: "string", description: "Specific finding about this area" },
                    recommendation: { type: "string", description: "Actionable recommendation if status is fail or warning" },
                  },
              ...
              required_actions: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    regulation: { type: "string", description: "Which regulation this action relates to" },
                    deadline: { type: "string", description: "Suggested timeline (e.g. Before launch, Within 30 days)" },
                  },
              ...
              compliant_aspects: {
                type: "array",
                description: "Aspects that are already compliant",
                items: { type: "string" },
              },
              prohibited_practices_detected: {
                type: "array",
                description: "Any prohibited practices detected (EU AI Act Art. 5 or GDPR violations)",
                items: { type: "string" },
              },
            },
            required: [
    ...
    const assessment = await callTool({
      system: SYSTEM_PROMPT,
      user: userPrompt,
      tool: tools[0],
      toolName: "submit_compliance_assessment",
    });
