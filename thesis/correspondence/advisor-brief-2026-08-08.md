...
- **Test suite**: 734 passing. Six failures plus four errors in `test_ai.py` are a **test-isolation defect** — the fixture let the developer's real local `.env` leak into the test environment — already fixed on an unmerged branch, where the same file passes 27/27.
- **CI**: red on `main` since 4 Aug for an unrelated reason — the `mcp` dependency is pinned only as `>=1.2.0`, and a newer release moved `mcp.server.fastmcp`, so a clean CI install fails to import where local installs succeed. A version ceiling fixes it.
