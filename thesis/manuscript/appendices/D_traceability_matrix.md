...
| H3 | Signal type affects how reliably feedback can be classified/routed | RQ3a | Gold-set per-field metrics; convergent in-repo LLM eval (`CLARA/…/evals/history.jsonl`), golden set grown 60→80 items with a per-item **language** field (60 en / 20 de; the German stratum is *authored*, not naturally occurring — disclosed); per-language metrics published via `run_live --publish` → `published_metrics.json`, served at `GET /model-card/metrics` | §5A.3–5A.5; §5A.7 | **measured** (two independent gold standards) |
| H4 | Journey stage materially shapes prioritisation | RQ4 | Interviews (the journey-stage field is labelled but **not scored** — §5A.1) | §5B | pending |
| H5 | Routing to the right owner is a recurring failure point | RQ1, RQ4 | Interviews (the owner field is labelled but **not scored** — §5A.1) | §5B | demonstrated + pending |
