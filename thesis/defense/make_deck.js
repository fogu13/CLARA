...
card(s, 0.6, 3.2, 5.95, 1.5, "The result that matters", "Triage accuracy depends on the method, and the method can change whose problems reach a human. On eight German-source signals whose label warrants escalation the floor escalates none, the learned model five and the contextual path seven: descriptive on that stratum, reported without ranking the predictors.");
...
bullets(s, [
  ...
  "The learned model joined the escalation equity table: 62.5% against 95.1% across the two strata, an observed 32.6 point gap on eight German-source items, reported as descriptive and without ranking the predictors.",
...
card(s, 0.6, 3.62, 7.4, 1.3, "Paired McNemar on the same items, testing accuracy",
  "Both learned methods beat the floor (p < 0.0001). The contextual path beats the learned model on sentiment in all four runs (p = 0.019 to 0.029) and ties it on severity; the production stage lands within three points of it on sentiment (8 discordant pairs to 3, p = 0.23) and one severity level below the seeds. The two production rows differ in model, batch size and run date together, so they compare deployment configurations, not the model alone.", { size: 11.5 });
...
s.addText("On the same eight German-source signals whose reference label warrants escalation: the floor escalates none, the learned model five, the contextual path seven (six in each repeat, 78 to 81% on the English side). The learned model ties the contextual path on severity accuracy; its stratum recalls differ by 32.6 points and the contextual path's by 0.3, descriptive figures on eight items. A significant test for one predictor beside a non-significant one for another does not compare the two gaps. The production stage escalates fewer in both strata.",
...
card(s, 6.75, 3.7, 5.95, 1.65, "The equity gap, once more",
  "Vodafone is the only new set with two source strata large enough to compare. There the learned model escalates German-source signals at 50% against English-source at 81.8%. Same direction as the learned model's observed 32.6 point gap on the thesis corpus, on draft labels, and descriptive in both cases.", { size: 13 });
...
const asks = [
  ...
  ["5. Does RQ3b stand on its own", "The learned model now shows an observed 32.6 point gap of its own on eight German-source signals, reported descriptively because within-predictor tests do not rank predictors, and the stratum is still eight signals. Is that enough as written, or do I need natural German-language data first? That decides whether relabelling the new corpora is required work."],
