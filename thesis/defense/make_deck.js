...
card(s, 0.6, 3.2, 5.95, 1.5, "The result that matters", "Triage accuracy depends on the method, and the method decides whose problems reach a human. Only the contextual path escalates German-source and English-source signals at close to the same rate.");
...
bullets(s, [
  ...
  "The learned model joined the escalation equity table and carries a 32.6 point gap of its own, so that finding is no longer about a weak baseline.",
...
s.addNotes("2 minutes, and for a Responsible AI thesis this is the most important results slide. Read the zero out loud and then pause. Then make the new point: the middle column was added on 20 August and it sharpens the finding, because a respectable learned model shows the same gap, so this is not an artefact of a weak baseline. Volunteer the bounds before she asks. Be precise if she pushes: the supported claim is the paired one on the same eight signals, not a cross-stratum claim that German is handled worse than English, which this corpus cannot establish. If she asks whether it generalises, the answer is no, the thesis says so, and the claim is about method choice on this corpus. One more honest caveat if she presses on the zero: across the three new datasets the same floor escalates German-source signals at 0.36 and 0.40, so the zero here is the low end of what a keyword floor does, not a fixed property of it.");
...
card(s, 6.75, 3.7, 5.95, 1.65, "The equity gap, once more",
  "Vodafone is the only new set with two source strata large enough to compare. There the learned model escalates German-source signals at 50% against English-source at 81.8%. Same direction, and nearly the same size as the 32.6 point gap on the thesis corpus.", { size: 13 });
...
bullets(s, [
  ...
  "Language is confounded with sector, and the texts are English paraphrases throughout, so every per-language result is a source-language stratum. One cross-language claim was retracted rather than defended.",
  ...
  "No inter-annotator agreement number exists behind either gold standard.",
  "The urgency exemplar effect was tested at n = 60, 80 and 100 without alpha spending, so the claim rests on three consecutive significant runs rather than on the first crossing.",
...
bullets(s, [
  ...
  "A second annotator on a stratified sample of the gold set, which produces the missing kappa.",
...
const asks = [
  ...
  ["3. Scope for the submission", "Of what is left, interviews, one live outcome measurement, a second annotator for the missing kappa, semantic scoring of two gold fields and free-form routing, which do you consider required and which is honest future work?"],
  ...
  ["5. Does RQ3b stand on its own", "The learned model now carries a 32.6 point gap of its own, which answers the strawman objection, and the German-source stratum is still eight signals. Is that enough as written, or do I need natural German-language data first? That decides whether relabelling the new corpora is required work."],
