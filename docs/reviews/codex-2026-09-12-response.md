...
6. **Split into PRs if wanted.** The branch holds one commit per area (C1; C2–C4; E2; E1; R1;
   manuscript; docs). They can be cherry-picked into separate PRs without re-resolving anything.
7. **Human validation** of the governance behaviour (an approver rejecting, editing and
...
- **Migration 016** (`apps/api/migrations/016_measurement_provenance.sql`): adds `origin` and
  ...
  the 013 §3 body plus the provenance keys. Idempotent (re-applied here without error). Rollback
